...
LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	{
	case WM_CREATE:
		hButton = CreateWindow(TEXT("BUTTON"), TEXT("ファイルを列挙"), WS_VISIBLE | WS_CHILD, 0, 0, 0, 0, hWnd, (HMENU)IDOK, ((LPCREATESTRUCT)lParam)->hInstance, 0);
	...
	case WM_COMMAND:
		if (LOWORD(wParam) == IDOK)
		{
			...
			HANDLE hSearch = FindFirstFile(TEXT("*.*"), &fd);
			...
			while (TRUE)
			{
				...
				SendMessage(hEdit, EM_REPLACESEL, 0, (LPARAM)szFileName);
				if (!FindNextFile(hSearch, &fd))
				{
					if (GetLastError() == ERROR_NO_MORE_FILES)
					{
						MessageBox(hWnd, TEXT("列挙が終了"), TEXT("終了"), MB_OK);
						break;
					}
					else
					{
						MessageBox(hWnd, TEXT("なんらかのエラー"), 0, MB_OK);
						break;
					}
				}
			}
